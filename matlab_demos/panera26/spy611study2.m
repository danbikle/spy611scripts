% /a/ks/b/matlab/panera26/spy611study2.m
...
for j=2:length(xy2.y2)
  ...
  if     xy2.upprob_l(i) > 0.51
    xy2.y2(j) = xy2.y2(i) * (1 + xy2.n1dg(i));
  elseif xy2.upprob_l(i) < 0.49
    xy2.y2(j) = xy2.y2(i) * (1 - xy2.n1dg(i));
  else
    xy2.y2(j) = xy2.y2(i) ;
  end

  if     xy2.upprob_g(i) > 0.51
    xy2.y3(j) = xy2.y3(i) * (1 + xy2.n1dg(i));
  elseif xy2.upprob_g(i) < 0.49
    xy2.y3(j) = xy2.y3(i) * (1 - xy2.n1dg(i));
  else
    xy2.y3(j) = xy2.y3(i) ;
  end

  if     xy2.upprob_p(i) > 0.51
    xy2.y4(j) = xy2.y4(i) * (1 + xy2.n1dg(i));
  elseif xy2.upprob_p(i) < 0.49
    xy2.y4(j) = xy2.y4(i) * (1 - xy2.n1dg(i));
  else
    xy2.y4(j) = xy2.y4(i) ;
  end
...
print (myfhandle, '-dpng', 'data/spy611study2.png');
