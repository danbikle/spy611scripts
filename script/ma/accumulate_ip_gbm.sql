--
-- ~/spy611/script/ma/accumulate_ip_gbm.sql
--

-- This script is called by:
-- ~/spy611/script/backtests/ma/ip_gbm.bash
...
DELETE FROM ip_n1dg WHERE algo = 'gbm' AND ydate IN (SELECT DISTINCT ydate FROM gbm_rpt_n1dg);
DELETE FROM ip_n1dg WHERE algo = 'gbm' AND ydate IN (SELECT DISTINCT ydate FROM gbm_rpt_n2dg);
DELETE FROM ip_n1dg WHERE algo = 'gbm' AND ydate IN (SELECT DISTINCT ydate FROM gbm_rpt_n1wg);
